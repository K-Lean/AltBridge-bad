...
public final class Player extends AndroidNonvisibleComponent implements Component, Deleteable, OnDestroySvcListener {
  ...
  public void Source(String path) {
    ...
    if (sourcePath.length() > 0) {
      ...
      try {
    	  if (isService) {
    		  MediaUtil.loadMediaPlayer(mp, formservice, sourcePath);
    	  } else {
  ...
  private void prepare() {
    ...
    try {
      ...
      playerState = 0;
      if (isService) {
    	  formservice.dispatchErrorOccurredEvent(this, "Source", ErrorMessages.ERROR_UNABLE_TO_PREPARE_MEDIA, sourcePath);
      } else {
    	  form.dispatchErrorOccurredEvent(this, "Source",
